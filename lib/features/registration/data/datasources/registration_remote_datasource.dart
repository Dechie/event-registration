...
abstract class RegistrationRemoteDataSource {
  ...
  Future<RegistrationResult> verifyOtp(String email, String otp);
  ...
  Future<Map<String, List<String>>> getLocationData(); // regions, cities, woredas
}
...
class RegistrationRemoteDataSourceImpl implements RegistrationRemoteDataSource {
  ...
  @override
  Future<RegistrationResponse> registerParticipant(RegistrationRequest request) async {
    try {
  ...
  @override
  Future<RegistrationResult> verifyOtp(String email, String otp) async {
    try {
      final response = await dioClient.post(
  ...
  Future<List<Session>> getAvailableSessions() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw ServerException(
          message: response.data['message'] ?? 'Failed to get available sessions',
  ...
  Future<Participant> updateParticipantInfo(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw ServerException(
          message: response.data['message'] ?? 'Failed to update participant info',
  ...
  Future<void> cancelRegistration(String participantId) async {
    try {
      final response = await dioClient.delete('/registration/participant/$participantId');
  ...
  Future<RegistrationResponse> getRegistrationStatus(String email) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw ServerException(
          message: response.data['message'] ?? 'Failed to get registration status',
      ...
      if (response.statusCode == 200) {
        ...
        return {
          ...
          'cities': (data['cities'] as List<dynamic>?)
                  ...
                  .toList() ??
              [],
          'woredas': (data['woredas'] as List<dynamic>?)
                  ?.map((e) => e.toString())
                  .toList() ??
              [],
        };
